...
MEMORY_CAPACITY = 50000     # size of replay buffer
...
class DDPG(object):
    ...
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        def get_actor(input_state_shape, name=''):
            ...
            :return: act
            """
            # input_state_shape = [None, 3]
            inputs = tf.keras.Input(input_state_shape, name='A_input')
            x = tf.keras.layers.Dense(units=30, activation=tf.nn.relu, kernel_initializer=W_init, bias_initializer=b_init, name='A_l1')(inputs)
        ...
        def get_critic(input_state_shape, input_action_shape, name=''):
            ...
            """
            # input_state_shape = [None, 3], input_action_shape = [None, 1]
            s = tf.keras.Input(input_state_shape, name='C_s_input')
            a = tf.keras.Input(input_action_shape, name='C_a_input')
            x = tf.keras.layers.concatenate(inputs=[s, a], axis=1)
            x = tf.keras.layers.Dense(units=60, activation=tf.nn.relu, kernel_initializer=W_init, bias_initializer=b_init, name='C_l1')(x)
        ...
        # self.R = tf.keras.Input([1], tf.float32, 'r')
